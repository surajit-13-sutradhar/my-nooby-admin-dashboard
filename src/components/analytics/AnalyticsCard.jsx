    ...
    return (
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
            {ANALYTICS_DATA.map((item, index) =>(
                // This is the card style that's going to be applied to all the items in the array ANALYTICS_DATA
                <motion.div
                    key={item.name}
                    className="bg-gray-800 bg-opacity-50 backfrop-filter backdrop-blur-lg shadow-lg rounded-xl p-6 border border-gray-700"
                ...
                >
                    <div className="flex items-center justify-between">
                        ...
                        <div className={`
                            p-3 rounded-full bg-opacity-20 ${item.change >= 0 ? "bg-green-300" : "bg-red-300"}  
                        `}>
                            <item.icon 
                                className={`size-6 ${item.change >=0 ? "text-green-600" : "text-red-500"}`}
                            />
                        </div>
